  ...
  // ─── 2. APPLICATION OBSERVABILITY ───
  {
    ...
    criteria: [
      ...
      },
      {
        ...
        query: "fetch spans | filter timestamp > now() - 2h | summarize services = countDistinct(dt.service.name), by: {trace.id} | filter services > 1 | summarize count()",
        queryB: "fetch spans | filter timestamp > now() - 2h | summarize countDistinct(trace.id)",
        thresholds: [{ min: 30 }, { min: 10 }, { min: 1 }],
      },
      {
  ...
  // ─── 7. AI OBSERVABILITY ───
  {
    ...
    criteria: [
      ...
      },
      {
        ...
        query: "fetch spans | filter timestamp > now() - 2h | filter isNotNull(gen_ai.system) or isNotNull(gen_ai.provider.name) or isNotNull(gen_ai.request.model) or isNotNull(gen_ai.operation.name) | filter isNotNull(gen_ai.usage.input_tokens) or isNotNull(gen_ai.usage.output_tokens) | summarize count()",
        queryB: "fetch spans | filter timestamp > now() - 2h | filter isNotNull(gen_ai.system) or isNotNull(gen_ai.provider.name) or isNotNull(gen_ai.request.model) or isNotNull(gen_ai.operation.name) | summarize count()",
        thresholds: [{ min: 80 }, { min: 50 }, { min: 1 }],
      },
      {
        id: "ai3", label: "AI provider diversity (%)",
        description: "Percentage of known AI providers detected vs expected (out of 5).",
        query: "fetch spans | filter timestamp > now() - 2h | filter isNotNull(gen_ai.system) or isNotNull(gen_ai.provider.name) or isNotNull(gen_ai.request.model) or isNotNull(gen_ai.operation.name) | fieldsAdd provider = coalesce(gen_ai.system, gen_ai.provider.name) | summarize countDistinct(provider)",
        queryB: "fetch spans | filter timestamp > now() - 2h | filter isNotNull(gen_ai.system) or isNotNull(gen_ai.provider.name) or isNotNull(gen_ai.request.model) or isNotNull(gen_ai.operation.name) | fieldsAdd provider = coalesce(gen_ai.system, gen_ai.provider.name) | summarize count = countDistinct(provider) | fields expected = 5",
      ...
      },
      {
        ...
        query: "fetch spans | filter timestamp > now() - 2h | filter isNotNull(gen_ai.system) or isNotNull(gen_ai.provider.name) or isNotNull(gen_ai.request.model) or isNotNull(gen_ai.operation.name) | filter isNotNull(gen_ai.agent.name) | summarize count()",
        queryB: "fetch spans | filter timestamp > now() - 2h | filter isNotNull(gen_ai.system) or isNotNull(gen_ai.provider.name) or isNotNull(gen_ai.request.model) or isNotNull(gen_ai.operation.name) | summarize count()",
        thresholds: [{ min: 30 }, { min: 10 }, { min: 1 }],
      },
      {
        ...
        query: "fetch spans | filter timestamp > now() - 2h | filter isNotNull(gen_ai.system) or isNotNull(gen_ai.provider.name) or isNotNull(gen_ai.request.model) or isNotNull(gen_ai.operation.name) | filter isNotNull(gen_ai.prompt) or isNotNull(gen_ai.completion) or isNotNull(gen_ai.input.messages) or isNotNull(gen_ai.output.messages) | summarize count()",
        queryB: "fetch spans | filter timestamp > now() - 2h | filter isNotNull(gen_ai.system) or isNotNull(gen_ai.provider.name) or isNotNull(gen_ai.request.model) or isNotNull(gen_ai.operation.name) | summarize count()",
        thresholds: [{ min: 80 }, { min: 50 }, { min: 1 }],
      },
      {
        ...
        query: 'fetch spans | filter timestamp > now() - 2h | filter isNotNull(gen_ai.system) or isNotNull(gen_ai.provider.name) or isNotNull(gen_ai.request.model) or isNotNull(gen_ai.operation.name) | filter status_code == "ERROR" | summarize count()',
        queryB: "fetch spans | filter timestamp > now() - 2h | filter isNotNull(gen_ai.system) or isNotNull(gen_ai.provider.name) or isNotNull(gen_ai.request.model) or isNotNull(gen_ai.operation.name) | summarize count()",
        thresholds: [{ min: 1 }],
      },
      {
        ...
        query: "fetch spans | filter timestamp > now() - 2h | filter isNotNull(gen_ai.system) or isNotNull(gen_ai.provider.name) or isNotNull(gen_ai.request.model) or isNotNull(gen_ai.operation.name) | filter isNotNull(gen_ai.guardrail) | summarize count()",
        queryB: "fetch spans | filter timestamp > now() - 2h | filter isNotNull(gen_ai.system) or isNotNull(gen_ai.provider.name) or isNotNull(gen_ai.request.model) or isNotNull(gen_ai.operation.name) | summarize count()",
        thresholds: [{ min: 50 }, { min: 20 }, { min: 1 }],
      },
      {
        ...
        query: "fetch spans | filter timestamp > now() - 2h | filter isNotNull(gen_ai.system) or isNotNull(gen_ai.provider.name) or isNotNull(gen_ai.request.model) or isNotNull(gen_ai.operation.name) | filter isNotNull(gen_ai.usage.cost) | summarize count()",
        queryB: "fetch spans | filter timestamp > now() - 2h | filter isNotNull(gen_ai.system) or isNotNull(gen_ai.provider.name) or isNotNull(gen_ai.request.model) or isNotNull(gen_ai.operation.name) | summarize count()",
        thresholds: [{ min: 50 }, { min: 20 }, { min: 1 }],
      },
      {
        ...
        query: "fetch spans | filter timestamp > now() - 2h | filter isNotNull(gen_ai.system) or isNotNull(gen_ai.provider.name) or isNotNull(gen_ai.request.model) or isNotNull(gen_ai.operation.name) | summarize count()",
        queryB: "fetch spans | filter timestamp > now() - 2h | summarize count()",
