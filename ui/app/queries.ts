    ...
    criteria: [
      {
        ...
        query: "fetch spans, from:now()-72h | filter isNotNull(gen_ai.system) or isNotNull(gen_ai.provider.name) or isNotNull(gen_ai.request.model) or isNotNull(gen_ai.operation.name) | summarize count = countDistinct(dt.entity.service) | fields count",
        queryB: "fetch dt.entity.service | summarize count()",
      ...
      },
      {
        ...
        queryB: "fetch spans, from:now()-72h | filter isNotNull(gen_ai.system) or isNotNull(gen_ai.provider.name) or isNotNull(gen_ai.request.model) or isNotNull(gen_ai.operation.name) | fieldsAdd provider = coalesce(gen_ai.system, gen_ai.provider.name) | summarize count = countDistinct(provider) | fields expected = 5",
